...
def get_MCL_data(args, preprocess_fns, epoch=0, tokenizer=None):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    preprocess_train, preprocess_val = preprocess_fns
    data = {}
    #for train, is_train=True
    if rank == 0:
        print('start dataset initialization')
    dataset=MCLDataset(args.train_data,transform=preprocess_train,tokenizer=tokenizer)
    if rank == 0:
        print('dataset initialized')
    num_samples = len(dataset)
    if args.MCL_label_path=='dummy':
        cluster_labels=torch.zeros(len(dataset),dtype=torch.long)
    else:
        cluster_labels=torch.load(args.MCL_label_path)
    cluster_num=int(torch.max(cluster_labels))+1
    if rank == 0:
        print('start sampler initialization')
    sampler=MCL_batch_sampler_DDP(cluster_labels,args.batch_size,cluster_num)
    if rank == 0:
